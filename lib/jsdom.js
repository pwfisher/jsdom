...
exports.env = exports.jsdom.env = function() {
  ...
  processHTML = function(err, html) {
    ...
    document = exports.html(html, null, options);
    if (!document) { return callback(new Error('JSDOM: a document object could not be created.')); }

    window = document.createWindow(),
    if (!window) { return callback(new Error('JSDOM: a window object could not be created.')); }
    if (!window.document) { return callback(new Error('JSDOM: a window object could not be created.')); }
    if (!window.document.implementation) { return callback(new Error('JSDOM: document has no implementation.')); }
